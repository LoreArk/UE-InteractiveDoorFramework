...
void UIDFPlayerInteractionComponent::StartInteraction()
{
	...
	if (TargetInteractable.IsValid() == false) return;
...
void UIDFPlayerInteractionComponent::InteractionTriggered()
{
	if (InteractedActor.IsValid() == false) return;

	if (InteractedActor.Get()->GetOwner() == nullptr) return;
	
	IIDFInteractable::Execute_InteractHeld(InteractedActor.Get()->GetOwner());
}
                                                  ...
                                                  FActorComponentTickFunction* ThisTickFunction)
{
	...
	UPhysicsHandleComponent* PlayerPhysicsHandle = PlayerCharacter->FindComponentByClass<UPhysicsHandleComponent>();
	if (!IsValid(PlayerPhysicsHandle))
		return;
	...
	if (IsValid(GrabbedComponent))
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("Interacted actor not valid!"));
		}

		USceneComponent* GrabHandle = PlayerCharacter->FindComponentByClass<USceneComponent>();
		if (!IsValid(GrabHandle))
			return;
		...
		FVector LerpedLocation = FMath::Lerp(TargetLocation, GrabHandleLocation, DeltaTime * 15.f);
