...
void AIDFDoubleDoorActor::OnConstruction(const FTransform& Transform)
{
	...
	if (HandleMesh.IsValid())
	{
		...
		if (bShowHandleA && HandleTransformRef)
		{
			...
			if (Handle1)
			{
				...
				Handle1->AttachToComponent(SecondDoorSkeletalMesh, FAttachmentTransformRules::KeepRelativeTransform);
		...
		if (bShowHandleB && HandleTransformRef)
		{
			...
			if (Handle2)
			{
				...
				Handle2->AttachToComponent(SecondDoorSkeletalMesh,FAttachmentTransformRules::KeepRelativeTransform);
	...
	SecondDoorSkeletalMesh->SetCanEverAffectNavigation(false);
...
void AIDFDoubleDoorActor::ClearDoubleDoorPreviousComponents()
{
	for (UActorComponent* Component : CreatedComponentsB)
	{
		if (!Component)
...
void AIDFDoubleDoorActor::PlayInteractionMontageAndSound(bool bShouldStopAllMontages)
{
	if (InteractionTargetComponent == CreatedDoorComponent)
	{
		...
		if (LockComponent->GetIsLocked())
		{
			...
			{
				SecondDoorSkeletalMesh->GetAnimInstance()->Montage_Play(LoadedLockedMontage, 1.0f, EMontagePlayReturnType::MontageLength, 0.0f, false);
			}
		}
		return;
	}
	...
	if (LoadedMontage)
	{
		SecondDoorSkeletalMesh->GetAnimInstance()->Montage_Play(InteractMontage.Get(), 1.0f, EMontagePlayReturnType::MontageLength, 0.15f, bShouldStopAllMontages);
	}
}
...
{
	if (TargetComponent == DoorSkeletalMesh && CreatedDoorComponent)
	{
		InteractionTargetComponent = CreatedDoorComponent;
	}
	else if (TargetComponent == SecondDoorSkeletalMesh && CreatedDoorComponentB)
	{
		InteractionTargetComponent = CreatedDoorComponentB;
	}
	else
	{
...
void AIDFDoubleDoorActor::HandleDoorMovementSounds(float Amount)
{
	if (InteractionTargetComponent == CreatedDoorComponent)
	{
		Super::HandleDoorMovementSounds(Amount);
		return;
	}

	const FTransform BoneTransform = SecondDoorSkeletalMesh->GetBoneTransform(DoorBoneName, ERelativeTransformSpace::RTS_ParentBoneSpace);
