...
public:
	...
	UPROPERTY(VisibleAnywhere, Category = "Door|Navigation")
	UNavModifierComponent* NavObstacle = nullptr;
	...
	/**< Reference to the montage to play when the player interacts and the door is locked.*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Door Settings")
	TSoftObjectPtr<UAnimMontage> LockedMontage;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "05 - Audio")
	TSoftObjectPtr<USoundCue> HandleInteractionSFX;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "05 - Audio")
	TSoftObjectPtr<USoundCue> LockedSFX;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "05 - Audio")
	TSoftObjectPtr<USoundCue> UnlockedSFX;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "05 - Audio")
	TSoftObjectPtr<USoundCue> MovementSFX;
	...
	/*< Minimum time to wait before activating the hold behavior (stop player input, door manual movement activation)*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Door Interaction")
	float InteractionHoldTimeThreshold = 0.1f;
	...
	virtual void HandleDoorMovementSounds(float Amount);
	...
	/*@brief Sets the Interaction Target, which is the component of this actor which received the interaction*/
...
protected:
	...
	virtual void ClearDoubleDoorPreviousComponents();
	...
	/*True if we have passed the inputs to the door for the manual movement, false otherwise*/
	UPROPERTY()
	bool bListeningToInputs;
	...
	/*< Seconds since the interaction started */
	UPROPERTY()
	float InteractionHeldDuration;
	...
	/*@brief Used OnConstruction, to apply the user chosen material*/
	UFUNCTION()
	virtual void ApplyMaterials(UStaticMeshComponent* MeshComp, const TArray<UMaterialInterface*>& Materials);

	/*Used OnConstruction, to align the blockers with the door mesh bounds*/
	virtual void AlignDoorBlockers();

	/*Disable both door blockers*/
	virtual void DisableBlocker(UBoxComponent* Blocker, bool bBackBlocker);

	/*Sets weather the Nav Obstacle should affect navigation (true if the door is locked, false otherwise) based on the current LockState*/
	virtual void UpdateNavObstacleAreaClass();

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "04 - HUD Messages")
	FString UnlockedHUDMessage = "Door unlocked.";

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "04 - HUD Messages")
	FString NoKeyTextMessage = "Door is locked: you need a key.";

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "04 - HUD Messages")
	FString WrongSideTextMessage = "Door is locked from the other side.";

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "04 - HUD Messages")
	FString BrokenTextMessage = "The handle is broken, I can't move it.";

	//Movement trackers to calculate when to play FXs inside the MoveDoor() function;
	bool bWasMoving;
	float PreviousDoorAngle = 0.f;
	int32 LastMovementDirection;
