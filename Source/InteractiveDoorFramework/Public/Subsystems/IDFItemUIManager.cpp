...
void UIDFItemUIManager::SetHUDMessageWidgetClass(TSubclassOf<UIDFHUDMessageWidget> WidgetClass)
{
    HUDMessageWidgetClass = WidgetClass;
}
...
void UIDFItemUIManager::ShowHUDMessage(const FText& MessageText)
{
    EnsureHUDMessageWidgetExists();

    if (HUDMessageWidget)
    {
        HUDMessageWidget->ShowMessage(MessageText);
    }
}
...
void UIDFItemUIManager::EnsureHUDMessageWidgetExists()
{
    if (!HUDMessageWidget && HUDMessageWidgetClass)
    {
        APlayerController* PC = UGameplayStatics::GetPlayerController(GetWorld(), 0);
        if (PC)
        {
            HUDMessageWidget = CreateWidget<UIDFHUDMessageWidget>(PC, HUDMessageWidgetClass);
            if (HUDMessageWidget)
            {
                HUDMessageWidget->AddToViewport(100);
            }
        }
    }
}
